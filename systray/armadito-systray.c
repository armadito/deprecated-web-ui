...
/* Support libarmadito-client API */
#include "apiclient.h"
#include "jutil.h"

#include "armadito-systray.h"

static struct option systray_option_defs[] = {
...
static void usage(void)
{
	...
	fprintf(stderr, "\n");
	fprintf(stderr, PRODUCT_NAME " antivirus systray\n");
	fprintf(stderr, "\n");
	fprintf(stderr, "Options:\n");
	fprintf(stderr, "  --help  -h                    print help and quit\n");
	fprintf(stderr, "  --version -V                  print program version\n");
	fprintf(stderr, "  --verbose -v                  output armadito infos\n");
	fprintf(stderr, "  --debug -d                    print HTTP trafic\n");
	fprintf(stderr, "  --port=PORT | -p PORT         TCP port to connect to "
	                PRODUCT_NAME "-AV (default is " S_DEFAULT_PORT ")\n");
	fprintf(stderr, "\n");

	exit(1);
}

static void parse_options(int argc, char **argv, systray_datas *datas)
{
	datas->port = DEFAULT_PORT;
	datas->verbose = 0;

	while (1) {
		int c, option_index = 0;

		c = getopt_long(argc, argv, "hdVvp:", systray_option_defs, &option_index);
		...
		case 'd': /* debug */
			...
			break;
		case 'p': /* port */
			datas->port = (unsigned short)atoi(optarg);
			break;
		case '?':
...
static void setup_client(systray_datas *datas)
{
	int end_of_info = 0;
	struct api_client *client;

	/* Start up Rest API client */
	client = api_client_new(datas->port, datas->debug);
	if (!client) {
		fprintf(stderr, "cannot allocate memory as api client\n");
		return;
	}

	datas->client = client;
	datas->status = ARMADITO_STATUS_NOT_REGISTERED;
	datas->error  = NULL;

	if (api_client_register(client) != 0) {
		datas->error = api_client_get_error(client);
	...
	if (api_client_call(client, "/status", NULL, NULL) != 0) {
		...
		datas->status |= ARMADITO_STATUS_NO_DAEMON;
		return;
	}
	...
	while (!end_of_info) {
		...
		if (api_client_call(client, "/event", NULL, &j_response) != 0) {
			...
			datas->status |= ARMADITO_STATUS_DAEMON_EVENT_FAILURE;
			return;
		}
	...
	datas->status = ARMADITO_STATUS_NORMAL;
}

void stop_client(systray_datas *datas)
{
	struct api_client *client = datas->client;

	if ( !(datas->status & ARMADITO_STATUS_NOT_REGISTERED) &&
	     api_client_unregister(client) != 0)
...
{
	systray_datas *datas = (systray_datas *)malloc(sizeof(systray_datas));
	struct api_client *client;
	...
	parse_options(argc, argv, datas);

	datas->setup_client = (systray_callback) setup_client ;
	datas->stop_client  = (systray_callback) stop_client ;

	launch_systray_app(datas);
